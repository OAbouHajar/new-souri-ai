...
class SearchIndexManager:
    ...
    async def build_embeddings_file(
        ...
        # Max ~7500 tokens ≈ 15000 chars (conservative for non-Latin text)
        MAX_CHARS_PER_TOKEN = 15000
        sentence_tokens = []
        ...
        # Truncate any tokens that exceed the embedding model's context limit
        truncated = 0
        for idx, tok in enumerate(sentence_tokens):
            if len(tok) > MAX_CHARS_PER_TOKEN:
                sentence_tokens[idx] = tok[:MAX_CHARS_PER_TOKEN]
                truncated += 1
        if truncated:
            print(f"  Truncated {truncated} tokens exceeding {MAX_CHARS_PER_TOKEN} chars")
        ...
        print(f"  Total sentence tokens: {len(sentence_tokens)}, batches: {total_batches}")
        with open(output_file, 'w') as fp:
            ...
            for batch_num, i in enumerate(range(0, len(sentence_tokens), batch_size), 1):
                batch = sentence_tokens[i:i+batch_size]
                ...
                for attempt in range(6):
                    try:
                        ...
                        if "429" in str(e) and attempt < 5:
                            ...
                            print(f"  Rate limited on batch {batch_num}/{total_batches}, retrying in {wait}s (attempt {attempt+1}/5)...")
                            await _asyncio.sleep(wait)
                        else:
                            raise
                for token, float_data in zip(batch, emedding):
                    writer.writerow({'token': token, 'embedding': json.dumps(float_data['embedding'])})
                print(f"  Batch {batch_num}/{total_batches} done ({len(batch)} tokens)")
